...
class QueryBuilder implements PromiseLike<{ data: any; error: any; count: number | null }> {
  ...
  private async resolveEmbeds(
    rows: Row[],
    parsed: ParsedSelect,
    currentTable: string = this.table
  ): Promise<void> {
    ...
    for (const embed of parsed.embeds) {
      ...
      if (fk) {
        ...
        const own = (FK_MAP[currentTable] || []).find((e) => e.column === fk);
        if (own) embedTable = own.foreignTable;
      } else {
        const fwd = (FK_MAP[currentTable] || []).find((e) => e.foreignTable === embed.table);
        if (fwd) fk = fwd.column;
      }
      if (fk) {
        ...
        // resolve nested embeds in the context of the related table
        ...
        const edge = this.findReverseEdge(embedTable, currentTable);
        const fkCol = edge?.column ?? `${singularize(currentTable)}_id`;
  ...
  private embedColumns(
    ...
    // Nested forward embeds need FK columns on this row (e.g. product_id for products(...))
    if (currentTable) {
      for (const e of parsed.embeds) {
        if (e.fkColumn) continue;
        const fwd = (FK_MAP[currentTable] || []).find((x) => x.foreignTable === e.table);
