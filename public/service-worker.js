// Baddies Curves - network-first pages; no HTML shell cache (playbook section 16)
const CACHE_VERSION = 'sw-v2.5-baddies';
...
self.addEventListener('push', (event) => {
  ...
  const options = {
    body: data.body || 'New update from Baddies Curves',
  ...
  event.waitUntil(
    self.registration.showNotification(data.title || 'Baddies Curves', options)
  );
});
